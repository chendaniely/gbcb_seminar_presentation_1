...
\subsection[Epidemiology]{Epidemiology}
    ...
    \begin{frame}[Basic2] \frametitle{Epidemiology}
        \begin{columns}
            ...
            \end{column}
            \begin{column}{0.5\textwidth}
                $\vcenter{\centering\includegraphics[width=0.5\linewidth]{../figures/epi-matters}}$
            \end{column}
        \end{columns}
    \end{frame}
...
\subsection{Theory of Reasoned Action}
    ...
    \end{frame}
%    
%    The TRA is a prominent health behavior theory that will be used as the theoretical foundation for the neural network.  This theory states that a behavior is determined by an individuals intention to perform a particular behavior.  In turn, intention is determined by the individual's attitudes and social context, both of with are governed by a set of beliefs.  In other words: beliefs $\rightarrow$ (attitudes \& social context) $\rightarrow$ intention $\rightarrow$ behavior.  We adapt the TRA to a computational artificial neural network model using constraint satisfaction.  
%    
%    \subsection{Constraint Satisfaction}
%    Constraint satisfaction (CS) puts a positive and negative component that represents the activation of a belief in an individual that is inversely proportional to one another.  The algorithm will then consolidate the inhibitory or excitatory ties between beliefs, the constraint, to calculate an individual's intention for a particular behavior \cite{Orr2013}.  Due to the dynamic, non-linear, and unequal interconnections and weights between beliefs, the neural network is an appropriate computational framework for the TRA.
%    %individual's attitude towards a behavior is determined by beliefs of the outcome and the intention to perform a particular behavior.
